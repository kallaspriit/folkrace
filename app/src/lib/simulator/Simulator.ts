...
export class Simulator {
  ...
  private tickGamepad(_info: TickInfo) {
    if (!this.gamepad) {
      return;
    }
    ...
    const speedInput = Math.abs(this.gamepad.axes[3]) > deadzone ? this.gamepad.axes[3] : 0;
    const omegaInput = Math.abs(this.gamepad.axes[0]) > deadzone ? this.gamepad.axes[0] : 0;
  ...
  private tickPathFinder(_info: TickInfo) {
  ...
  private renderForeground({ layer }: FrameInfo) {
    ...
    this.statistics.statistics.forEach((statistic, i) => {
      layer.drawGraph({
        name: `${statistic.options.name}: ${statistic.getLatest().toFixed(statistic.options.decimalPlaces || 0)}${
