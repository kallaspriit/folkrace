...
const destinationPath = path.join(__dirname, "..", "..", "android", "folkbot", "src", "main", "assets");
...
(async () => {
  ...
  if (useWatchMode) {
    chokidar
      ...
      .on("all", async (event, filename: string) => {
        const relativeFilename = filename.replace(sourcePath, "").substr(1).replace(/\\/g, "/");

        process.stdout.write(
          chalk.reset(`> ${chalk.bold(pad(event, 9))} ${chalk.bold(pad(relativeFilename, 60, true))}\n`),
        );
  ...
  process.stdout.write(`${chalk.bgGreen.black(" SYNC COMPLETE ")} in ${chalk.bold(`${pad(timeTaken, 4)}ms`)}\n`);
...
const syncDebounced = debounce(async () => {
  ...
  process.stdout.write(`${chalk.bgGreen.black(" SYNC COMPLETE ")} in ${chalk.bold(`${pad(timeTaken, 4, true)}ms`)}\n`);
}, 1000);
